...
The user reports having tested computer use before the review fixes. The new event-monitor path and static native notice still need live acceptance: capture a Revit dialog with two text fields, quickly tap Tab between watchdog ticks, then attempt typing from the old observation. Expect zero input and a fresh observation; interrupting after a typing batch must retain an unknown outcome without replay. Also repeat normal click/type, recovery and Stop with the rebuilt helper. Automated hook lifecycle tests do not inject physical input into Revit.

Build `dotnet/Revcode.Desktop/Revcode.Desktop.csproj -c Release`, run `npm run test:native`, and run `node --test scripts/desktop-workflow.test.mjs`. Desktop regression cases cover coordinate mapping, receipt deduplication, Win32 input layout, lease expiry and Stop, stale/passive frames, partial insertion without replay, the unknown fence, and an overlay appearing during message pumping. Driver tests exercise image-review staging around the production controller, fresh-image matching, cancellation during staging/capture, passive capture, and preservation of receipts when post-action capture fails. Native tests also cover completed taps invalidating old evidence, exclusion of only helper-tagged injection, interruption between typing batches, and hook installation/disposal across turns. CI runs these tests, builds the helper, and checks driver syntax. The injected dispatch callbacks test control flow without live Windows input; actual mutex/hotkey behavior and Revit remain live acceptance checks.
...
See [DESKTOP-PROTOCOL.md](DESKTOP-PROTOCOL.md) for installed usage and the optional manual driver. Earlier live checks exercised foreground acquisition and basic view orientation/style changes; the current observation-recovery and cancellation fixes still need live acceptance. Capture the ribbon, Project Browser, GPU viewport, hover menu, and an owned dialog; compare the experimental GDI baseline with HWND Windows.Graphics.Capture. In a disposable project, locate the search field from the fresh PNG, click, type a unique Unicode string, verify its display, select it with CTRL+A, clear with BACKSPACE, and capture again. Repeat at 100%, 150%, 200%, negative monitor origins, mixed DPI, and after movement/resizing. Record exact Revit/Windows versions, backend, screenshot IDs, action receipts, and every failure.

For recovery acceptance, exercise delayed menu opening, Options open/close, model latency beyond the 15-second frame limit, Stop during initial launch and a queued native start, and user interruption after a dialog transition. Verify refresh never resends input and native unknown activation input fences both desktop and API mutations, including after host restart. Automated regressions cover the host recovery path and the native cancellation policy, but do not execute DesktopSession or its STA queue against live windows.
