    ...
    function walk(parent: Root | Element) {
      for (const node of parent.children) {
        ...
        if (Array.isArray(props.ariaDescribedBy))
          props.ariaDescribedBy = props.ariaDescribedBy.map((id) => prefix + id);
        walk(node);
      }
    }
    walk(tree);
...
function navigateFragment(event: MouseEvent<HTMLAnchorElement>) {
  ...
  const target = Array.from(
    reply?.querySelectorAll<HTMLElement>("[id]") ?? [],
...
const components: Components = {
  ...
  a: ({ node: _node, href, ...props }) => (
    <a
      ...
      onClick={navigateFragment}
      onAuxClick={navigateFragment}
    />
  ),
  table: ({ children }) => (
  ...
  return (
    <div className="message-markdown">
      <Markdown
        ...
        rehypePlugins={[[scopeMarkdownIds, `${id}-`]]}
        skipHtml
