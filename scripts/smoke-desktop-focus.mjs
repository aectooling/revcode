...
const desktop = new DesktopController(client, directory, () => {}, () => {});
await desktop.init();
try {
 for (let attempt = 1; attempt <= (repeat ? 2 : 1); attempt++) {
  ...
  const frame = await desktop.beginTurn(null).observe({});
  ...
  console.log(JSON.stringify({ attempt, title: frame.title, actionable: frame.actionable, width: frame.width, height: frame.height, directory }));
